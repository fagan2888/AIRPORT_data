...
##############################
#  covert the data to path
#  visual the path
##############################
path = os.path.abspath(os.path.join(os.path.dirname("__file__"), os.path.pardir))
infile = path + '\\data\\oneday.csv'
outfile = path + '.\\carpath_time.csv'
...
def readfile():
    ...
    for line in lines[1:]:
        ...
        if cno != '':
            data_list.append([no, cno, lng, lat, dtime])
...
def carpath():
    ...
    for cno in range(1, maxcno + 1):
        for line in data_list:
            ...
            dtime = line[4]
            if line[1] == str(cno):
                car_list[cno - 1].append([no, lng, lat, dtime])
            else:
                pass
    return car_list
...
def pathfile(carlist):
    ...
    for ocar in carlist:
        ...
        for i in range(len(ocar) - 1):
            ...
            p1 = ocar[i + 1]
            outlist = [str(carlist.index(ocar) + 1), ocar[1][3], p0[0], p1[0],
                       '"[' + p0[1] + ',' + p0[2] + '],[' + p1[1] + ',' + p1[2] + ']"']
            string = ','.join(outlist) + '\n'
            print string,
